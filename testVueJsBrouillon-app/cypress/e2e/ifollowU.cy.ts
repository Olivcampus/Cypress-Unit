...
describe.only('Follow', () => {
  beforeEach(() => {
    ...
    // Intercept the API call to get the user's followers and return an empty list
    cy.intercept('GET', /profiles\/\S+\/followers/, { statusCode: 200, body: { followers: [] } }).as('getUserFollowers')

    cy.fixture('article.json').then(article => {
      article.article.author.username = 'test'
